...
def align_feature_columns(feature_df: pd.DataFrame, feature_columns: List[str]) -> pd.DataFrame:
...
def main() -> None:
    ...
    feature_df = build_test_features(test_df, k=args.k)
    feature_columns = metadata["feature_columns"]
    inference_frame = align_feature_columns(feature_df, feature_columns)
    ...
    action_pred = assert_integer_like(action_predictor.predict(inference_frame), "actionId")
    point_pred = assert_integer_like(point_predictor.predict(inference_frame), "pointId")
